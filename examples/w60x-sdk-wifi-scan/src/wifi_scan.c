...
void UserMain(void)
{
	...
	tls_wifi_scan_result_cb_register(&WiFiScanDone);

	int res = tls_wifi_scan();
	if(res == WM_SUCCESS) {
		printf("WiFi scan start OK.\n");
	} else if(res == WM_WIFI_SCANNING_BUSY) {
		printf("WiFi scanner was still busy..\n");
	} else {
		printf("WiFi scanning fail: %d\n", res);
	}

	while(1) {
		tls_os_time_delay(2000 / portTICK_PERIOD_MS);
		printf("scan done\n");
	}
}
